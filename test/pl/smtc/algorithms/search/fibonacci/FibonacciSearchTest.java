...
class FibonacciSearchTest extends BaseTestOut {
    ...
    @Test
    void executeShouldInvokeFibonacciSearchExample() {
        fibonacciSearch.execute();
    ...
    @Test
    void getNameShouldReturnFibonacciSearchString() {
        assertEquals("Fibonacci search", fibonacciSearch.getName());
    }
    ...
    void findShouldThrowExceptionWhenNoElementIsFound() {
        ...
        assertThrows(NoSuchElementException.class, () -> fibonacciSearch.find(testList, nonExistingElement));
    }
    ...
    void findShouldCorrectlySearchExistingInputElement(List<Integer> toFind, int item, int result) {
        assertEquals(result, fibonacciSearch.find(toFind, item));
    }
    ...
    private static Stream<Arguments> searchElementInSortedArray() {
        return Stream.of(
                ...
                Arguments.of(Arrays.asList(1, 1, 1, 1, 1, 1), 1, 2),
                Arguments.of(Arrays.asList(-37, -37, 0, 0, 73, 73), 0, 2),
                Arguments.of(Arrays.asList(-11, -11, -11, -11, -11, -11), -11, 2));
    }
}