...
class ExponentialSearchTest extends BaseTestOut {
    ...
    @Test
    void executeShouldInvokeExponentialSearchExample() {
        exponentialSearch.execute();
    ...
    @Test
    void getNameShouldReturnExponentialSearchString() {
        assertEquals("Exponential search", exponentialSearch.getName());
    }
    ...
    void findShouldThrowExceptionWhenNoElementIsFound() {
        ...
        assertThrows(NoSuchElementException.class, () -> exponentialSearch.find(testList, nonExistingElement));
    }
    ...
    void findShouldCorrectlySearchExistingInputElement(List<Integer> toFind, int item, int result) {
        assertEquals(result, exponentialSearch.find(toFind, item));
    }
