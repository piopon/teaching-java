...
class CocktailSortTest extends BaseTestOut {
    ...
    @Test
    void executeShouldInvokeCocktailSortExample() {
        cocktailSort.execute();
    ...
    @Test
    void getNameShouldReturnCocktailSortString() {
        assertEquals("Cocktail sort", cocktailSort.getName());
    }

    @ParameterizedTest(name = "Cocktail sorted array: {1}")
    @MethodSource("sortTestData")
    void sortShouldCorrectlySortInputTable(List<Integer> toSort, List<Integer> sorted) {
        assertEquals(sorted, cocktailSort.sort(toSort));
    }
