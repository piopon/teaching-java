...
class CountingSortTest extends BaseTestOut {
    ...
    @Test
    void executeShouldInvokeCountingSortExample() {
    ...
    @Test
    void getNameShouldReturnCountingSortString() {
        assertEquals("Counting sort", countingSort.getName());
    }

    @ParameterizedTest(name = "Counting sorted array: {1}")
    @MethodSource("sortTestData")
    void sortShouldCorrectlySortInputTable(List<Integer> toSort, List<Integer> sorted) {
        assertEquals(sorted, countingSort.sort(toSort));
    }
