...
class BitonicSortTest extends BaseTestOut {
    ...
    @Test
    void executeShouldInvokeBitonicSortExample() {
        bitonicSort.execute();
        String output = getOutput();
        assertTrue(output.contains("TO SORT: [10, -9, 1, 0, -3, 4, -1, 3, 1, 2]"));
        assertTrue(output.contains("SORTED:  [-9, -3, -1, 1, 0, 1, 2, 3, 4, 10]"));
    }

    @Test
    void getNameShouldReturnBitonicSortString() {
        assertEquals("Bitonic sort", bitonicSort.getName());
    }

    @ParameterizedTest(name = "Bitonic sorted array: {1}")
    @MethodSource("sortTestData")
    void sortShouldCorrectlySortInputTable(List<Integer> toSort, List<Integer> sorted) {
        assertEquals(sorted, bitonicSort.sort(toSort));
    }
